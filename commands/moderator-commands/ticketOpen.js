...
module.exports.run = async (bot, message, args) => {
   ...
   // Setups 
    const ticketsCategory = message.guild.channels.cache.find(c => c.name.includes("Support Rooms") && c.type === "category")
    ...
    const newTicketErrEmbed = new Discord.MessageEmbed()
     ...
     .setFooter(message.author.tag + " | Peace Keeper", message.author.displayAvatarURL({dynamic: true, size: 1024}))
    ;
    const noLoggingChannelErrEmbed = new Discord.MessageEmbed()
     .setColor('FF6961')
     .setTitle("**error!**")
     .addField("No ticket logging channel found!.", "Please make a channel named `#ticket-logs` to log all tickets.")
     .setTimestamp()
     .setFooter(message.author.tag + " | Peace Keeper", message.author.displayAvatarURL({dynamic: true, size: 1024}))
    ;
    ...
    if(!logChannel) return message.channel.send(noLoggingChannelErrEmbed);
